...
import { TriviaEngine, type RandomSource } from '../../src/games/trivia/engine/TriviaEngine';
import { mergeStoredRooms, recoverRoomStorage, serializeStoredRooms } from '../../src/platform/rooms/roomStorageRecovery';
...
const KEYS = { primary: PRIMARY_KEY, backup: BACKUP_KEY };
...
describe('room storage recovery', () => {
  ...
  it('repairs a valid but incomplete primary snapshot from backup', () => {
    ...
    const recovered = recoverRoomStorage(storage, KEYS);
  ...
  it('recovers a real active room and preserves host and player reconnect credentials', () => {
    const engine = new TriviaEngine(new FixedRandom(), 60_000);
    ...
    recoverRoomStorage(localStorage, KEYS);

    const restoredEngine = new TriviaEngine(new FixedRandom(), 60_000);
  ...
  it('quarantines a malformed newer primary record and restores the valid backup', () => {
    ...
    const restored = new TriviaEngine(new FixedRandom(), 60_000);
  ...
  it('does not throw when browser storage access is blocked', () => {
    ...
    expect(recoverRoomStorage(blocked, KEYS)).toEqual([]);
  });
  ...
  it('skips a structurally corrupt room record instead of crashing engine startup', () => {
    ...
    expect(() => new TriviaEngine(new FixedRandom(), 60_000)).not.toThrow();
  ...
  it('migrates legacy arrays to the versioned schema and quarantines malformed payloads', () => {
    ...
    recoverRoomStorage(storage, KEYS);
    ...
    expect(recoverRoomStorage(storage, KEYS)).toHaveLength(1);
