...
import { TriviaEngine } from '../../src/games/trivia/engine/TriviaEngine';
...
} from '../../src/platform/players/playerCustomization';
...
describe('player customization', () => {
  ...
  it('persists customization through disconnect, reconnect, and room recovery', () => {
    const engine = new TriviaEngine(random, 60_000);
    ...
    const recovered = new TriviaEngine(random, 60_000).snapshot(host.roomCode).players[0];
  ...
  it('migrates an old saved player title without breaking room recovery', () => {
    ...
    const restored = new TriviaEngine(random, 60_000);
  ...
  it.each(SCORE_EFFECTS.map((effect) => effect.id))('persists the %s score effect', (scoreEffect) => {
    ...
    expect(new TriviaEngine(random, 60_000).snapshot(host.roomCode).players[0].scoreEffect).toBe(scoreEffect);
  });
});
