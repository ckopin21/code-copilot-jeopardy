...
import type { GameSettings } from '../../src/games/trivia/types';
...
function addPlayer(engine: TriviaEngine, roomCode: string, name = 'Alex') {
  return engine.joinPlayer(roomCode, { name, avatar: '🚀', accent: '#93c5fd' });
}

function firstUnused(engine: TriviaEngine, roomCode: string) {
  return engine.snapshot(roomCode).board!.questions.find((question) => !question.used)!;
}

function finishBoardWithoutScoring(engine: TriviaEngine, roomCode: string, hostToken: string): void {
...
describe('TriviaEngine production state', () => {
  ...
  it('falls back to the recovery snapshot when the primary room snapshot is corrupt', () => {
    ...
    const recovered = new TriviaEngine(new FixedRandom(), 60_000).snapshot(host.roomCode);
