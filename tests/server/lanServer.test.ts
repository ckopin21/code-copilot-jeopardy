...
import type { ScoreEvent } from '../../src/games/trivia/server';
import type { TriviaEngine } from '../../src/games/trivia/engine/TriviaEngine';
import { createSocketRuntime, type SocketRuntime } from '../../src/platform/net/socket';
import type { RoomSnapshot } from '../../src/games/trivia/types';
...
let game: RoomServer | null = null;
const trivia = () => game!.engine<TriviaEngine>('trivia');
let url = '';
...
async function start(): Promise<void> {
  server = createServer();
  game = createRoomServer({ httpServer: server, storage: new MemoryStorage() });
...
describe('LAN Socket.IO game server', () => {
  it('uses the production socket adapter for Host, two phones, Presentation, scoring, and identity replay', async () => {
    ...
    const tile = trivia().snapshot(host.roomCode).board!.questions[0];
    ...
    const started = trivia().snapshot(host.roomCode).gameStartedAt;
    ...
    await scoreReceived;
    const before = trivia().snapshot(host.roomCode);
    ...
    const after = trivia().snapshot(host.roomCode);
  ...
  it('runs a multi-client clue through the authoritative server and broadcasts one score effect', async () => {
    ...
    const board = trivia().snapshot(host.roomCode).board!;
    ...
    const context = trivia().snapshot(host.roomCode);
    ...
    expect(trivia().snapshot(host.roomCode).currentQuestion?.buzzWinnerId).toBe(one.playerId);
    ...
    const score = trivia().snapshot(host.roomCode).players.find((player) => player.id === one.playerId)!.score;
    expect(score).toBeGreaterThan(0);
    await request(hostSocket, 'host:resolve-answer', hostPayload(host, { playerId: one.playerId, correct: true }), scoreRequestId);
    expect(trivia().snapshot(host.roomCode).players.find((player) => player.id === one.playerId)?.score).toBe(score);
    ...
    expect(trivia().snapshot(host.roomCode).phase).toBe('board');
    ...
    expect(trivia().snapshot(anotherHost.roomCode).players).toHaveLength(0);
    ...
    expect(trivia().snapshot(anotherHost.roomCode).phase).toBe('lobby');
  });
  ...
  it('grades Free Response and a Daily Double through socket requests', async () => {
    ...
    const question = trivia().snapshot(host.roomCode).currentQuestion!;
    const started = trivia().snapshot(host.roomCode).gameStartedAt;
    ...
    await request(oneSocket, 'player:text-response', textPayload, 'text-once');
    expect(Object.keys(trivia().snapshot(host.roomCode).currentQuestion?.textResponses ?? {})).toHaveLength(1);
    ...
    expect(trivia().snapshot(host.roomCode).currentQuestion?.answerRevealed).toBe(true);
    await request(hostSocket, 'host:confirm-text-grades', hostPayload(host));
    const graded = trivia().snapshot(host.roomCode);
    ...
    await request(hostSocket, 'host:start-game', hostPayload(host));
    const daily = trivia().snapshot(host.roomCode).board!.questions.find((entry) => entry.dailyDouble)!;
    ...
    const dailyState = trivia().snapshot(host.roomCode);
    ...
    await request(oneSocket, 'player:daily-double-wager', wagerPayload, 'daily-wager-once');
    expect(trivia().snapshot(host.roomCode).currentQuestion?.wager).toBe(100);
    await request(hostSocket, 'host:reveal-answer', hostPayload(host));
    await request(hostSocket, 'host:resolve-answer', hostPayload(host, { playerId: one.playerId, correct: true }));
    expect(trivia().snapshot(host.roomCode).players.find((player) => player.id === one.playerId)?.score).toBe(100);
  });
  ...
  it('tells the Host when saving rooms fails and when it recovers', async () => {
    ...
    server = createServer();
    game = createRoomServer({ httpServer: server, storage });
    await new Promise<void>((resolve) => server!.listen(0, '127.0.0.1', resolve));
    const address = server.address();
    if (!address || typeof address === 'string') throw new Error('Test HTTP server has no TCP address');
    url = `http://127.0.0.1:${address.port}`;
  ...
  it('keeps unrevealed Final answers private on Presentation and supports a room-preserving rematch', async () => {
    ...
    await request(displaySocket, 'presentation:join', { roomCode: host.roomCode, presentationToken: presentationToken(host) });
    await request(hostSocket, 'host:start-game', hostPayload(host));
    while (trivia().snapshot(host.roomCode).phase === 'board') {
      const tile = trivia().snapshot(host.roomCode).board!.questions.find((entry) => !entry.used)!;
      await request(hostSocket, 'host:select-question', hostPayload(host, { questionId: tile.questionId }));
      await request(hostSocket, 'host:reveal-answer', hostPayload(host));
      await request(hostSocket, 'host:advance-board', hostPayload(host));
    }
    expect(trivia().snapshot(host.roomCode).phase).toBe('final-category');
    ...
    expect(trivia().snapshot(host.roomCode).phase).toBe('recap');
    await request(hostSocket, 'host:reset-game', hostPayload(host));
    const rematch = trivia().snapshot(host.roomCode);
