...
`src/ui-layout-contract.css` is the final shared layout contract. It defines the reusable spacing, avatar, control, and status-lane tokens; it is the only place allowed to choose the internal layout of a live player identity. A player card has three normal-flow tracks—avatar, identity/score, and `PlayerStatusStack`—so adding a streak or question-turn pill cannot change the identity region's size or switch to an absolute offset. `PlayerStatusStack` owns only status order and its vertical gap; the card owns the lane. Live names, scores, and compact presentation identities are single-line ellipsis fields, while long-form question, answer, and podium copy remains allowed to wrap in its own constrained container. Board result modifier pills are flex-flow siblings of their played value, never absolutely positioned from it.
