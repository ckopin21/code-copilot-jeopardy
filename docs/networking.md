...
Each successfully opened host peer claims an ownership lease scoped to its room code. Independent rooms can therefore run concurrently, while a second host for the same room deterministically displaces the old authority. Host mutations, timer ticks, stale-phone cleanup, and incoming controller requests are accepted only while that peer still owns its room lease. A displaced same-room host closes stale controller channels and can no longer mutate or rewrite that room's persisted state.
...
The browser keeps both a primary persisted room snapshot and a previous valid recovery snapshot. They use a versioned storage envelope; legacy array saves migrate on recovery. Records are validated before their freshness is compared, so a malformed newer primary cannot defeat a valid backup. Invalid raw payloads are quarantined under a timestamped local-storage key before the primary is repaired.
