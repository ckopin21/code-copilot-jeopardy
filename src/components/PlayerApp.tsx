...
export function PlayerApp() {
  ...
  return <main className={`player-phone-v2 ${me.onFire?'phone-fire':''} ${me.isCold?'phone-cold':''}`} style={{'--accent':me.accent} as React.CSSProperties}>
    ...
    {(room.phase === 'question' || room.phase === 'daily-double-question') && current && <section className="phone-question-stage-v2">
      <div className="phone-question-v2"><div className="section-kicker">{current.category}</div>{questionMultiplier > 1 && !current.dailyDouble && <div className={`inline-modifier x${questionMultiplier}`}>{current.baseValue} × {questionMultiplier} = {current.effectiveValue} POINTS</div>}{current.dailyDouble && current.wager !== null && <div className="phone-public-wager"><small>{dailyDoublePlayer?.avatar} {dailyDoublePlayer?.name} LOCKED IN</small><strong>{current.wager.toLocaleString()} WAGER</strong><span>{room.settings.dailyDoubleStacksWithMultiplier && questionMultiplier > 1 ? `${current.wager.toLocaleString()} × ${questionMultiplier} = ` : ''}{pointsAtStake.toLocaleString()} POINTS IN PLAY</span></div>}<h2>{current.text || 'Wager in progress…'}</h2><Timer timer={room.timer} serverNow={room.serverNow}/></div>
