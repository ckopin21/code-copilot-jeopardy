...
export function PresentationApp() {
  ...
  return <main className="presentation-shell">
    ...
    {(room.phase === 'question' || room.phase === 'daily-double-question') && current && <section className={`presentation-question ${current.dailyDouble ? 'daily-double-v2' : ''}`}><article>
      ...
      {current.buzzWinnerId && (() => { const player = room.players.find((candidate) => candidate.id === current.buzzWinnerId); if (!player) return null; const customization = normalizePlayerCustomization(player); const title = getPlayerTitleLabel(customization.title); return <div className="winner-chip presentation-winner" style={{ '--accent': player.accent } as React.CSSProperties}><PlayerAvatar avatarId={player.avatarId} fallback={player.avatar} frameStyle={customization.frameStyle} accent={player.accent} /><span>{player.name}{title && <small className="player-title-badge">{title}</small>}</span></div>; })()}
    ...
    {room.phase === 'recap' && <section className="presentation-center presentation-recap"><div className="section-kicker gold">GAME COMPLETE</div>{winners.length === 1 ? (() => { const winner = winners[0]; const customization = normalizePlayerCustomization(winner); const title = getPlayerTitleLabel(customization.title); return <div className="presentation-winner-identity" data-victory-effect={customization.victoryEffect} style={{ '--accent': winner.accent } as React.CSSProperties}><PlayerAvatar avatarId={winner.avatarId} fallback={winner.avatar} frameStyle={customization.frameStyle} accent={winner.accent} /><h1>{winner.name}</h1>{title && <span className="player-title-badge">{title}</span>}</div>; })() : <h1>TIE GAME</h1>}<div className="presentation-scores-v2">{[...resultPlayers].sort((a,b)=>b.score-a.score).map((player)=>{ const customization = normalizePlayerCustomization(player); return <div key={player.id} style={{ '--accent': player.accent } as React.CSSProperties}><span><PlayerAvatar avatarId={player.avatarId} fallback={player.avatar} frameStyle={customization.frameStyle} accent={player.accent} /> {player.name}</span><strong>{player.score.toLocaleString()}</strong></div>; })}</div></section>}
