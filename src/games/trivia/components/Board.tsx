import type { BoardQuestionResult, BoardState } from '../types';
import { questionModifierLabels, resultModifierLabels } from '../ui/boardResultPresentation';
            ...
            <div className="used-result-list">{questionResults.length > 2
              ...
              : questionResults.map((result) => {
                ...
                return <span className={`used-result-chip ${result.correct ? 'correct' : 'wrong'}`} key={result.playerId} style={{ '--accent': result.playerAccent ?? '#ffd166' } as React.CSSProperties}>
                  <b><PlayerAvatar avatarId={result.playerAvatarId} fallback={result.playerAvatar} accent={result.playerAccent} className="used-result-avatar-art" autoCenter /> <span>{result.playerName}</span></b>
