...
import { PlayerStatusStack } from './PlayerStatusStack';
      ...
      {visiblePlayers.map((player) => {
        ...
        return (
          <section data-player-id={player.id} key={player.id} data-score-effect={customization.scoreEffect} data-frame={customization.frameStyle} className={`player-card showcase-player-card ${activeId === player.id ? 'is-active' : ''} ${isTurn ? 'is-turn' : ''} ${isTurn && turnLabel ? 'has-turn-label' : ''} ${player.onFire ? 'is-fire' : ''} ${player.isCold ? 'is-cold' : ''}`} style={{ '--accent': player.accent } as React.CSSProperties}>
            <div className="player-avatar-large"><PlayerAvatar avatarId={player.avatarId} fallback={player.avatar} frameStyle={customization.frameStyle} accent={player.accent} autoCenter /></div>
            ...
            <PlayerStatusStack
              streak={streakBadge}
              turn={turnBadge}
              fallback={player.finalWagerSubmitted ? <span className="status-pill neutral">WAGER LOCKED</span> : <span className="status-pill neutral">{player.positiveStreak > 0 ? `STREAK ${player.positiveStreak}` : 'READY'}</span>}
            />
          </section>
        );
      })}
