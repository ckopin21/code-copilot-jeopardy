import { COLD_STREAK_THRESHOLD, DEFAULT_SETTINGS, GAME_LENGTH_CONFIG } from '../config';
import type { BoardQuestion, GameSettings, Player, Question, QuestionPack, RoomSnapshot, RoomState } from '../types';
...
import { autoGradeAnswer } from '../grading';
import { packMap, packSupportsGameMode, packsForGameMode } from '../packs';
import { calculateComebackAward } from '../rules/comebackScoring';
import { finalWagerRules } from '../rules/finalWagerRules';
import { gameModeAllowsDailyDoubles, gameModePenalizesTypedTimeout, isGameMode, responseModeForGameMode } from '../gameModes';
import { randomId } from '../../../platform/ids';
import { normalizePlayerCustomization, type PlayerCustomizationFields } from '../../../platform/players/playerCustomization';
import { isValidStoredRoomPayload, recoverRoomStorage, serializeStoredRooms, type RoomStorageKeys } from '../../../platform/rooms/roomStorageRecovery';
import { randomToken, ROOM_CODE_ALPHABET, secureEqual } from '../../../platform/rooms/tokens';
...
/** Storage keys predate the rename from the peer-to-peer engine; changing them would orphan saved rooms. */
const STORAGE_KEYS: RoomStorageKeys = { primary: 'blue-stage-p2p-engine-v2', backup: 'blue-stage-p2p-engine-v2-backup' };
const ROOM_TTL_MS = 24 * 60 * 60 * 1000;
const emptyTimer = () => ({ running: false, durationMs: null, endsAt: null, remainingMs: null });

export interface TriviaEngineOptions {
  /** Lets the room server keep codes unique across every game. */
...
function loadRooms(storage: Storage): { rooms: RoomRecord[]; readable: boolean } {
  try {
    return {
      rooms: recoverRoomStorage(storage, STORAGE_KEYS) as unknown as RoomRecord[],
      readable: true
    };
  } catch {
...
export class TriviaEngine implements RoomEngine<RoomSnapshot> {
  private rooms = new Map<string, RoomRecord>();
  private seenQuestionIds = new Set<string>();
  private persistenceHealthy = true;

  constructor(private readonly random: RandomSource = new MathRandomSource(), private readonly roomTtlMs = ROOM_TTL_MS, private readonly storage: Storage = localStorage, private readonly options: TriviaEngineOptions = {}) {
    const now = Date.now();
  ...
  private persist(): void {
    try {
      ...
      if (previous && previous !== serialized && isValidStoredRoomPayload(previous)) this.storage.setItem(STORAGE_KEYS.backup, previous);
  ...
  private code(): string {
    for (let attempt = 0; attempt < 100; attempt += 1) {
      let value = '';
      for (let index = 0; index < 5; index += 1) value += ROOM_CODE_ALPHABET[Math.floor(this.random.next() * ROOM_CODE_ALPHABET.length)];
      if (!this.rooms.has(value) && !this.options.isRoomCodeTaken?.(value)) return value;
    }
    throw new Error('Could not allocate room code');
  }
  hasRoom(roomCode: string): boolean {
    const record = this.rooms.get(roomCode.toUpperCase());
    return Boolean(record && record.state.expiresAt > Date.now());
  }
