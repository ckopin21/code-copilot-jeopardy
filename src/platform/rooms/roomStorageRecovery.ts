/** Where a game keeps its saved rooms. Each game must use its own keys. */
export interface RoomStorageKeys { primary: string; backup: string }
...
export function recoverRoomStorage(storage: Storage, keys: RoomStorageKeys): StoredRoom[] {
  const primaryRaw = safeGetItem(storage, keys.primary);
  const backupRaw = safeGetItem(storage, keys.backup);
  ...
  if (parsedPrimary.malformed && primaryRaw) safeSetItem(storage, `${keys.primary}-corrupt-${Date.now()}-primary`, primaryRaw);
  if (parsedBackup.malformed && backupRaw) safeSetItem(storage, `${keys.primary}-corrupt-${Date.now()}-backup`, backupRaw);
  if (primaryRaw !== serialized) safeSetItem(storage, keys.primary, serialized);
  return merged;
}
