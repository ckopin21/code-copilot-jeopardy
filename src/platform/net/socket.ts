...
import type { BaseRoomSnapshot, HostRoomCredentials, PlayerJoinCredentials } from '../rooms/types';
...
export function createSocketRuntime(options: SocketRuntimeOptions = {}): SocketRuntime {
  ...
  let lastRoom: BaseRoomSnapshot | null = null;
  ...
  for (const event of EVENT_NAMES) {
    transport.on(event, (data: unknown) => {
      if (event === 'room:state') {
        lastRoom = data as BaseRoomSnapshot;
